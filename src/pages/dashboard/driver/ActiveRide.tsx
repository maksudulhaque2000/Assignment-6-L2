...
const ActiveRide = () => {
  ...
  if (!ride) {
    return (
      <div className="p-4">
        ...
        <p className="mt-4">You do not have any ongoing rides at the moment.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4">
      <h1 className="text-2xl font-bold mb-4">Active Ride Details</h1>
      <div className="bg-white p-6 rounded-lg shadow-md">
        <p><strong>Rider:</strong> {ride.riderId?.name || 'N/A'}</p>
        <p><strong>Status:</strong> <span className="font-semibold capitalize">{ride.status}</span></p>
        <p className="text-sm"><strong>From:</strong> {ride.pickupLocation.coordinates.join(', ')}</p>
        <p className="text-sm"><strong>To:</strong> {ride.destinationLocation.coordinates.join(', ')}</p>
